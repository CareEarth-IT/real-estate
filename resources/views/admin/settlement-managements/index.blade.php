                        ...
                        @foreach ($settlementManagements as $settlementManagement)
                            ...
                            >
                                ...
                                <td class="sticky-col sticky-col-last px-3 py-3 whitespace-nowrap">
                                    @php
                                        $contractPeriod = $settlementManagement->contract_date
                                            ?? $settlementManagement->customer?->contract_period;
                                        $contractPeriodDisplay = $contractPeriod instanceof \DateTimeInterface
                                            ? $contractPeriod->format('Y/m/d')
                                            : ($contractPeriod ?: '—');
                                    @endphp
                                    {{ $contractPeriodDisplay }}
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    @switch($settlementManagement->fee_type)
                                        @case('broker_fee') 仲介手数料 @break
                                        @case('advertising') 広告料 @break
                                        @default {{ $settlementManagement->fee_type ?? '—' }}
                                    @endswitch
                                </td>
